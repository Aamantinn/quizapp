...
const displayQuestions = document.getElementById("display-answers");
const successMessage = document.getElementById("usernameSuccessMessage");
const answerRestartButton = document.getElementById("answer-restart");
...
startButton === null || startButton === void 0 ? void 0 : startButton.addEventListener("click", startQuiz);
...
function startQuiz() {
    ...
    // Store username
    const username = (usernameInput === null || usernameInput === void 0 ? void 0 : usernameInput.value) || storedUsername || "Anonymous";
    ...
    if (answerRestartButton)
        answerRestartButton.classList.add("hidden");
...
function showQuestion(question) {
    ...
    question.answers.forEach((answer, index) => {
        ...
        button.classList.add("block", "w-full", "cursor-pointer", "border-2", "md:text-lg", "text-sm", "rounded-lg", "p-3", "text-left", "font-semibold", "border-secondary");
        ...
        button.addEventListener("click", () => {
            ...
            if (button && !button.classList.contains("answered")) {
                ...
                if (button && answer.correct) {
                    ...
                    if (answerButtons)
                        (_a = answerButtons
                            .querySelector(`button[value="${question.answers.findIndex((a) => a.correct)}"]`)) === null || _a === void 0 ? void 0 : _a.classList.add("bg-green", "text-white");
                }
                if (answer.correct) {
                    score++;
                }
...
function resetState() {
    if (answerButtons)
        answerButtons.querySelectorAll("button").forEach((btn) => {
            btn.classList.remove("answered", "bg-green", "bg-red", "text-white");
        });
}
// Event listener for the next button
nextButton === null || nextButton === void 0 ? void 0 : nextButton.addEventListener("click", () => {
    const selectedButton = answerButtons === null || answerButtons === void 0 ? void 0 : answerButtons.querySelector(".selected");
    if (selectedButton) {
        // Store the user's answer in localStorage before moving to the next question
        localStorage.setItem(`userAnswer_${currentQuestionIndex}`, selectedButton.value);
        resetState();
...
restartButton === null || restartButton === void 0 ? void 0 : restartButton.addEventListener("click", resetQuiz);
...
function endQuiz() {
    ...
    if (answerRestartButton)
        answerRestartButton.classList.remove("hidden");
    ...
    localStorage.setItem("latestUsername", username);
    restartButton === null || restartButton === void 0 ? void 0 : restartButton.classList.remove("hidden");
...
function showAnswers() {
    ...
    if (displayQuestions) {
        ...
        questions.forEach((question, qIndex) => {
            ...
            button.classList.add("block", "w-full", "cursor-pointer", "border-2", "md:text-lg", "text-sm", "rounded-lg", "p-3", "text-left", "font-semibold");
            ...
            button.innerText = `${qIndex + 1}. ${question.question} - ${userAnswerIndex !== undefined && userAnswerIndex !== null
...
function openModal() {
    ...
    const storedUsername = localStorage.getItem("latestUsername");
    usernameInput;
    const username = (usernameInput === null || usernameInput === void 0 ? void 0 : usernameInput.value) || storedUsername;
...
function submitNewQuestion() {
    ...
    const correctOption = document.getElementById("option4").value;
